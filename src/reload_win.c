...
void load_renderer(struct renderer *ren)
{
	...
	if (ren->lib_handle != NULL) {
		ren->load_functions = (typeof(ren->load_functions))GetProcAddress(ren->lib_handle, "load_functions");
		...
		printf("couldn't load renderer dll");
	}
}
...
void load_editor_lib(struct editor *editor)
{
	...
	if (editor->lib_handle != NULL) {
		editor->load_functions =
			(typeof(editor->load_functions))GetProcAddress(editor->lib_handle, "load_functions");
